...
;
var FilterPaneBase = /** @class */ (function (_super) {
    ...
    FilterPaneBase.prototype.renderFilterShelf = function (filter, index) {
        ...
        var filterComponent;
        if (predicate_1.isFieldRangePredicate(filter)) {
            if (fieldDef.type === expandedtype_1.ExpandedType.TEMPORAL) {
                domain = filter_1.getDefaultTimeRange(domain, timeUnit);
            }
            ...
            if (timeUnit) {
                domain = filter_1.getDefaultList(timeUnit);
            }
...
var filterShelfTarget = {
    drop: function (props, monitor) {
        ...
        if (filter_1.filterHasField(props.filters, filter.field)) {
